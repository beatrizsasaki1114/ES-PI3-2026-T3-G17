...
export const listStartups = onCall(
  ...
  // Se o usuário enviar um Stage inválido,
  // a função retorna um erro, evitando dados incorretos
  ...
  const startups = (await listStartupItems())
    ...
    .sort((left, right) =>
      left.NomeStartup.localeCompare(right.NomeStartup)
    );
